...
The function `drop_na()` from the `dplyr` package (a subset of the `tidyverse`) allows you to remove rows from a dataset that contain NAs.
...
pi %>% sin() %>% cos()
```
This method allows us to read code from left to right rather than inside out. The shortcut for a pipe on windows in Rstudio should be set to Ctrl + Alt + M by default (Cmd + Shift + M for macbook users).
